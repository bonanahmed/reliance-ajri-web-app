            ...
            <div class="tab-steps-item {{ $loop->index+1 > 1 ? 'd-none' : '' }}">
                <div class="row">
                    <div class="col text-center card-custom">
                        ...
                        @if($item->image)<img style="width:100%;border-radius: 1rem;height:300px;object-fit: cover; position:relative; z-index:10" src="{{ asset('storage/'.$item->image) }}" alt="{{$item->title}}" title="{{$item->title}}">
                        @endif
                    </div>
